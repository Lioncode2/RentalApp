...
public class RentalService : IRentalService
{
    ...
    // Creates a rental request with three business rule checks:
    // 1. End date must be after start date
    // 2. No overlapping bookings for the same item
    // 3. Total price = daily rate x number of days
    ...
    // Moves rental status from Requested to Approved
    // Only works if rental is currently in Requested state
    ...
    // Moves rental status from Requested to Rejected
    public async Task<Rental> RejectRentalAsync(int rentalId)
    {
        var rental = await _rentalRepo.GetByIdAsync(rentalId) ?? throw new KeyNotFoundException();
        if (rental.Status != RentalStatus.Requested)
            throw new InvalidOperationException("Can only reject requested rentals");
        rental.Status = RentalStatus.Rejected;
        return await _rentalRepo.UpdateAsync(rental);
    }

    // Marks a rental as completed once the item has been returned
